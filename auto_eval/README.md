...
1. Complete the `taskweaver_config.json` under the `project` folder with your LLM endpoint configuration. The auto evaluator reads LLM settings from the same project config used by TaskWeaver.
...
It is a list of dicts with `"role"` and `"content"` keys representing the chat history between the virtual user and the agent.
...
class YourVirtualUser(VirtualUser):
      def __init__(self, task_description: str, llm_client, model_name: str):
...
def auto_evaluate_for_your_agent(
    ...
    # Create the VirtualUser and Evaluator (both share the same LLM client)
    # You need to create an OpenAI/AzureOpenAI client and pass it in
    vuser = YourVirtualUser(task_description, llm_client, model_name)
    evaluator = Evaluator(llm_client, model_name)
